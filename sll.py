...
class LinkedList:
    ...
    def insert_at_index(self, index: int, value: object) -> None:
        ...
        if list.length() == 0 or index == 0:
            new_link._next = cur._next
            cur._next = new_link

        # For inserting at last position
        elif index == list.length():
            list.add_back(value)
    ...
    def remove_at_index(self, index: int) -> None:
        ...
        """
        if index < 0 or index >= list.length() or list.length() == 0:
            raise SLLException()
    ...
    def slice(self, start_index: int, size: int) -> object:
        """
        Returns a new LinkedList object that contains the requested number of nodesfrom the original list starting with the node located at the requested start index
        """
        ...
        if start_index < 0 or size < 0 or (
                start_index + size) > self.length() or start_index >= self.length() or size == 0:
            raise SLLException()
